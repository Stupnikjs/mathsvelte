...
function draw3Axis(x:number, width:number) {
  ...
  function drawLine( arr1:[number, number], arr2:[number, number], color:string){
    var xAxis = draw.line( arr1[0] , arr1[1], arr2[0], arr2[1]).stroke({ color: color, width: width });
  }

  // Draw the x-axis vector (red)
  drawLine([originX, originY], [originX + vectorLength, originY], "red")
  drawLine([originX, originY], [originX, originY - vectorLength], "blue")
  ...
  var zAxis = draw.line(originX,originY, originX - vectorLength, originY + vectorLength).stroke({ color: '#2196f3', width: width});
